...
function Profile.createProfile(plrOrKey, settings)
	Assert(
		typeof(plrOrKey) == "Instance" and plrOrKey.ClassName == "Player" or typeof(plrOrKey) == "string",
...
function Profile:_rawSetSettings(settings)
	Assert(
		...
		"Invalid argument #1 (settings.maxGetRequests must be type 'number')"
	)
	Assert(
		not settings.maxSaveRequests or typeof(settings.maxSaveRequests) == "number",
		"Invalid argument #1 (settings.maxSaveRequests must be type 'number')"
	)
	Assert(
		not settings.maxDeleteRequests or typeof(settings.maxDeleteRequests) == "number",
		"Invalid argument #1 (settings.maxDeleteRequests must be type 'number')"
	)
	Assert(
		not settings.maxVersionRequests or typeof(settings.maxVersionRequests) == "number",
		"Invalid argument #1 (settings.maxVersionRequests must be type 'number')"
	)
	Assert(
		not settings.maxSaveVersionRequests or typeof(settings.maxSaveVersionRequests) == "number",
		"Invalid argument #1 (settings.maxSaveVersionRequests must be type 'number')"
	)
	Assert(
		not settings.maxDataVersionRequests or typeof(settings.maxDataVersionRequests) == "number",
		"Invalid argument #1 (settings.maxDataVersionRequests must be type 'number')"
	)
	Assert(
		not settings.maxSaveDataVersionRequests or typeof(settings.maxSaveDataVersionRequests) == "number",
		"Invalid argument #1 (settings.maxSaveDataVersionRequests must be type 'number')"
	)
	Assert(
		not settings.studioSave or typeof(settings.studioSave) == "boolean",
		"Invalid argument #1 (settings.studioSave must be type 'boolean')"
	)
	Assert(
		not settings.default or (typeof(settings.default) == "nil" or typeof(settings.default) == "table"),
		"Invalid argument #1 (settings.default must be type 'nil' or 'table')"
	)
	Assert(
		not settings.private or (typeof(settings.private) == "boolean" or typeof(settings.private) == "table"),
		"Invalid argument #1 (settings.private must be type 'boolean' or 'table')"
	)
	Assert(
		not settings.mergedData or (typeof(settings.mergedData) == "nil" or typeof(settings.mergedData) == "table"),
		"Invalid argument #1 (settings.mergedData must be type 'nil' or 'table')"
	)
	...
	Assert(
		...
		"Invalid argument #1 (settings.migrators must be type 'table' and have a minimum of 1 migrator)"
	)
	...
	if settings.serializer then
		Assert(
			typeof(settings.serializer) == "function",
			"Invalid argument #1 (settings.serializer must be type 'function')"
		)
		newSettings.serializer = settings.serializer
	end
	if settings.deserializer then
		Assert(
			typeof(settings.deserializer) == "function",
			"Invalid argument #1 (settings.deserializer must be type 'function')"
		)
...
function Profile:_rawSet(profileData, exact)
	...
	elseif typeof(profileData) == "table" and typeof(oldProfileData) == "table" then
		newProfileData = AssignWithNone({}, oldProfileData, profileData)
	else
		newProfileData = profileData
	end
...
function Profile:_saveVersion(profileData)
	if self.shouldSaveProfileData then
		...
		local success, data =
			self.datastore:saveIterator("requestSaveVersion", value, self.settings.maxSaveVersionRequests)
...
function Profile:_saveDataVersion()
	if self.shouldSaveProfileData then
		...
		local success, data =
			self.datastore:saveIterator("requestSaveDataVersion", dataVersion, self.settings.maxSaveDataVersionRequests)
...
function Profile:delete()
	...
	if success then
		...
		local success, data =
			self.datastore:requestIterator("requestDeleteDataVersion", self.settings.maxDeleteRequests)
		if success then
...
function Profile:setVisibility(plrTbl)
	...
	self:_log(
		"["
			...
			.. (typeof(plrTbl) == "table" and "plrTbl" or plrTbl)
			.. ")",
		1
	)
end
